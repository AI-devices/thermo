...
abstract class AppStyle {
  ...
  static boxShadowBottomRight([bool inset = false]) {
    if (inset) {
      // Inset illusion: darker color, negative spread, opposite offset
      return const BoxShadow(
        color: Color.fromARGB(255, 176, 176, 177),
        offset: Offset(-2.0, -2.0), // Flip for inner
        blurRadius: 3.0,
        spreadRadius: -1.0, // Negative pulls inward
      );
    }
    return const BoxShadow(
      color: Color.fromARGB(255, 176, 176, 177),
  ...
  static boxShadowTopLeft([bool inset = false]) {
    if (inset) {
      // Inset: lighter color, negative spread, adjusted offset
      return BoxShadow(
        color: Colors.white.withOpacity(0.8),
        offset: const Offset(2.0, 2.0), // Flip for inner
        blurRadius: 5.0,
        spreadRadius: -1.0,
      );
    }
  ...
  static Icon getIconBattery(int? charge) {
    if (charge == null)
      return const Icon(Icons.battery_unknown, color: Colors.white);
    if (charge < 5) return const Icon(Icons.battery_0_bar, color: Colors.white);
    if (charge < 15)
      return const Icon(Icons.battery_1_bar, color: Colors.white);
    if (charge < 30)
      return const Icon(Icons.battery_2_bar, color: Colors.white);
    if (charge < 45)
      return const Icon(Icons.battery_3_bar, color: Colors.white);
    if (charge < 60)
      return const Icon(Icons.battery_4_bar, color: Colors.white);
    if (charge < 75)
      return const Icon(Icons.battery_5_bar, color: Colors.white);
    if (charge < 90)
      return const Icon(Icons.battery_6_bar, color: Colors.white);
